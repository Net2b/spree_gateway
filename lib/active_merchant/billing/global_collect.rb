module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    class GlobalCollect < Gateway
      ...
      self.supported_countries = %w(AD AE AT AU BD BE BG BN CA CH CY CZ DE DK EE
                                    EG ES FI FR GB GI GR HK HU ID IE IL IM IN IS
                                    IT JO KW LB LI LK LT LU LV MC MT MU MV MX MY
                                    NL NO NZ OM PH PL PT QA RO SA SE SG SI SK SM
                                    TR TT UM US VA VN ZA)

      # The card types supported by the payment gateway
      self.supported_cardtypes = [:visa, :master, :american_express, :discover,
                                  :jcb, :diners_club, :laser, :solo, :maestro,
                                  :dankort]
    ...
    private
      ...
      def add_merchant_details(post, options)
        ...
        post[:merchant_reference]   = options[:merchant_reference] || rand(Time.now.to_i).to_s.slice(0..29)
      end
      ...
      def add_invoice(post, options)
        post[:order_id]             = options[:order_id] || options[:orderid] || rand(Time.now.to_i).to_s.slice(0..9)
      ...
      def add_creditcard(post, creditcard)
        ...
        post[:payment_product_id] = payment_product_id(creditcard.cc_type)
